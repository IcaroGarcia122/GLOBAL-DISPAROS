          ...
          {plans.map((plan, idx) => {
            ...
            return (
              <div 
                ...
                className="animate-fade-in opacity-0 relative group"
              >
                {plan.recommended && (
                ...
                <div className={`h-full transition-all duration-500 flex flex-col ${isTrimensal ? 'laser-container scale-105 shadow-[0_40px_100px_rgba(255,255,255,0.1)]' : 'bg-[#1c2433] border border-white/5 hover:border-brand-500/20 shadow-2xl rounded-[3.5rem]'}`}>
                  <div className={`laser-inner p-12 rounded-[3.5rem] flex flex-col h-full ${isTrimensal ? 'bg-brand-600' : 'bg-[#1c2433]'}`}>
                    ...
                    <div className="mb-12">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-6xl font-[950] text-white italic tracking-tighter">{plan.price}</span>
                    ...
                    <button className={`w-full py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all transform hover:-translate-y-1 active:scale-95 shadow-xl ${plan.recommended ? 'bg-white text-brand-600 hover:bg-slate-100' : 'bg-brand-600 text-white hover:bg-brand-500 shadow-brand-600/20'}`}>
                      {plan.cta}
                    </button>
                  </div>
                </div>
